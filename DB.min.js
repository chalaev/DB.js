...
case 'forEach':
_e._A=map1(_d.children,c=>c.cloneNode(true));
...
case 'forEach':
var _A=map1(_d.children,_be=>_be.cloneNode(true));
while(_d.firstChild) _d.removeChild(_d.firstChild);
for(var _w in value)
_A.forEach(_be=>_d.appendChild(treat(_be.cloneNode(true),value[_w],_w)))}}
_d.removeAttribute('db')})}}
forEach1(_d.children,c=>treat(c,_M,_t));
return _d}
return{
'scanHTML':function(_ds=[document]){forEach1(_ds,_be=>treat(_be,null,null))},
...
forEach1(_be._A,_bg=>_be._d.appendChild(
