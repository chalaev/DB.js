...
if(_be.sort){let _bg=_be.value;_bg.sort(_be.sort());_Z(_be,_bg)}});
...
if(_d.getAttribute){
let _a=_d.getAttribute("db");
if(_a){_a.split(',').forEach(function(_db){
let _b=_db.split(':'),_c=_b[0],
...
case 'forEach':
let _A=Array.from(_d.children).map(_be=>_be.cloneNode(true));
while(_d.firstChild) _d.removeChild(_d.firstChild);
for(const _w in value)
...
let _E=Array.isArray(_be)? []:{};
for(const _w in _be) _E[_w]=_H(_be[_w]);
return _E}
...
return _P};
_h.drop=function(_w){
let _z="string"==typeof _w? parseInt(_w):_w,_r=_J.value[_z];
